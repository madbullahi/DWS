...
ps_ww <- qza_to_phyloseq(
  ...
  metadata = "MetadataWastewater.tsv"
)
...
```{r}
set.seed(123)
# rarefy the data to 5000 reads per sample
...
WW <- ggplot(
  tax_rel_abun_pf_gen_top10, aes(x = Day, y = Abundance, fill = Genus)+
    ...
    labs(x = "Day", y = "Relative Abundance", title = "Genus Rel. Abun") +
    theme(
      axis.text.x = element_text(size = 10, angle = 90, hjust = 1, vjust = 0.5),
      axis.title.x = element_text(size = 10, face = "bold", vjust = 0.5, hjust = 1),
...
```{r}
WW<- ggplot(
  tax_rel_abun_pf_gen_top10, aes(x = Day, y = Abundance, fill = Treatment)
)+
...
wilcox_genus = tax_rel_abun_pf_gen_top10)
{
