...
```{r}
merge_less_than_top_ps <- function(ps_object_rare2, top = 10) { transformed <- transform_sample_counts(ps_object_rare2, function(OTU) OTU/sum(OTU)) 
otu.table <- as.data.frame(otu_table(transformed))
otu.sort <- otu.table[order(rowMeans(otu.table), decreasing = TRUE),]
otu.list <- row.names(otu.sort[(top + 1):nrow(otu.sort),])
merged <- merge_taxa(transformed, otu.list, 1)
...
ps_Gen_10 <- merge_less_than_top_ps(ps_gen, top = 10)
pf_family_10 <- merge_less_than_top_ps(ps_family, top = 10)

```

```{r}
# convert to dataframe
df_gen_top10 <- psmelt(ps_Gen_10) %>%
  filter(Abundance > 0) %>%
  arrange(Genus)
df_family_top10 <- psmelt(pf_family_10) %>%
  filter(Abundance > 0) %>%
  arrange(Family)
...
```{r}
#save the data
write.csv(df_gen_top10, "genus_top10.csv")
write.csv(df_family_top10, "family_top10.csv")
