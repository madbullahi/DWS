...
df_plot <- df_relative %>%
  mutate(Genus = ifelse(Genus %in% top_15_genera, Genus, "Other")) %>%
  group_by(Genotype, Day, Genus) %>%
...
df_plot <- df_plot %>%
  ...
  mutate(SE = sd(RelativeAbundance) / sqrt(n())) %>%
  ungroup()
...
main_plot <- ggplot(df_plot, aes(x = Genotype, y = RelativeAbundance, fill = Genus)) +
  geom_bar(stat = "identity", position = "stack") +
  geom_errorbar(aes(ymin = RelativeAbundance - SE, ymax = RelativeAbundance + SE), 
                width = 0.2, position = position_stack(vjust = 0.5)) +
  facet_wrap(~Day, ncol = 1) +
  scale_fill_viridis_d(option = "turbo") +
  labs(x = "Genotype", y = "Relative Abundance", title = "Genus Relative Abundance") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "right")
...
create_pie <- function(data, x_pos, y_pos, radius) {
  data$fraction <- data$RelativeAbundance / sum(data$RelativeAbundance)
  data$ymax <- cumsum(data$fraction)
  data$ymin <- c(0, head(data$ymax, n=-1))
  
  ggplot(data, aes(fill = Genus, ymax = ymax, ymin = ymin, xmax = 4, xmin = 3)) +
    geom_rect() +
    coord_polar(theta = "y") +
    xlim(c(0, 4)) +
    theme_void() +
...
# Create pie charts for each Genotype and Day combination
pie_charts <- df_plot %>%
  group_by(Genotype, Day) %>%
  group_map(~ create_pie(.x, 0, 0, 0.4))

# Combine main plot with pie charts
final_plot <- ggdraw(main_plot) +
  map2(pie_charts, seq_along(pie_charts), function(p, i) {
    draw_plot(p, x = 0.9, y = 1 - i * 0.2, width = 0.1, height = 0.1)
  })
...
# Save the plot
ggsave("genus_abundance_with_pies.png", final_plot, width = 12, height = 15, dpi = 300)


# Create pie charts for each Genotype and Day
